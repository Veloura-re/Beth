...
export const getDailyScans = async () => {
...
export const getAllScans = async () => {
  const { data, error } = await supabase.from('scans').select('*, agent:profiles!scans_agent_id_fkey(name, email)').order('timestamp', { ascending: false });
  if (error) throw new Error(error.message);
  return data;
};
